...
job_details_df   %>% kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width="100%",height="300px")
...
job_desc_corpus <- VCorpus(VectorSource(jobs_data$details))
...
```{r}
for (j in seq(job_desc)) {
    job_desc[[j]] <- gsub("/", " ", job_desc[[j]])
    job_desc[[j]] <- gsub("@", " ", job_desc[[j]])
    job_desc[[j]] <- gsub("\\|", " ", job_desc[[j]])
    job_desc[[j]] <- gsub("\u2028", " ", job_desc[[j]])  # This is an ascii character that did not translate, so it had to be removed.
}
