...
It seems that **AM WEST** has more delay rate especially for **Seattle** and **San Franscisco** with 30.8% and 40.3% respectively compared to **ALASKA** which has almost **half** the rates.

We have to get the total flights per state 

```{r}
total_flights <- mutate(spread_df, total_num = delayed + `on time`)
total_flights
```
get the highest number per airline company 
...
highest_num
```
It seams that **AM WEST** is domenating in **Phoenix** while **ALASKA** is in **Seattle**.
