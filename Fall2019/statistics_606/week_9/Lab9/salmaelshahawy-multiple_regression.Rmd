...
The two variable that i choose are age and gender. We can see here younger instructors are having higher scores.
...
```
In the Initial scatterplot ne dot represents many observations and this is fixed by atting jitter.
...
```
```{r}
correlation = cor(evals$bty_avg, evals$score)
correlation
```
 y(hat)=3.88+0.06xbty_avg
...
```{r}
DATA606::plot_ss(x= evals$bty_avg, y= jitter(evals$score), showSquares = TRUE)
```
...
score(hat)=(3.75+0.1724)+0.074×bty_avg 
...
summary(m_back)
```
score(hat)=3.77+0.17xethnicity_not_minority+0.21xgendermale−0.206xlanguage_non_english−0.006xage+0.004xcls_perc_eval+0.505xcls_credits_one_credit+0.05xbty_avg−0.19xpic_color_color
...
```{r}
ggplot2::qplot(x = .fitted, y = .stdresid, data = m_back, position = "jitter")
```

```{r}
ggplot2::qplot(sample = .stdresid, data = m_back) 
# + geom_abline()
```

Nearly normal residuals so data appears to satisfy the conditions for regression.
