...
Observational study. We can rephrase the question as: Is there an association between beauty and course evaluation score?
...
Score is left skewed. The distribution of score is between 2.3 and 5 with a median score of 4.3 and an IQR of less than 1.
...
I picked age and gender. We can see that younger instructors are having higher scores.
...
```
In the Initial scatterplot ne dot represents many observations 
...
```
\[
\bar{y}(score) = 3.88 + 0.066x(bty-avg)
\]
```{r}
corr <- cor(evals$bty_avg, evals$score)
corr
```

The equation means that to increase score, the `bty_avg` should increase by 0.066 value.
`bty_avg` is a statistically significant predictor of evaluation score with p-value close of 0.
...
```
Nearly normal residuals: The residuals appear to be nearly normal. Linearity: The relationship looks linear
...
Variability is constant so conditions for regression are pretty reasonable.
...
```
bty_avg is still significant and the addition of gender has changed the estimate from 0.066 to 0.074. 
...
\[
\begin{aligned}
\widehat{score} &= (3.747 + 0.172) + 0.0741 \times bty\_avg 
\end{aligned}
\]
Male professors would score 0.1724 higher than a female with the same beauty rating.
...
It was true, the number of professor variable `cls_profs`

13. Interpret the coefficient associated with the ethnicity variable.

When ethnicity is not minority the score tends to increase by 0.1234929.
...
```{r}
m_without_cls_profs = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_without_cls_profs)
```

The coefficients and p-values have changed slightly.The dropped variable had only minimal collinearity with the other explanatory variables.
...
```{r}
m_back <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval + cls_credits + bty_avg + pic_color, data = evals)
summary(m_back)
```
score_hat=3.77+0.17∗ethnicity_not_minority+0.21∗gendermale−0.206∗language_non_english−0.006∗age+0.004∗cls_perc_eval+0.505∗cls_credits_one_credit+0.05∗bty_avg−0.19∗pic_color_color

16. Verify that the conditions for this model are reasonable using diagnostic 
    plots.

```{r}
qplot(x = .fitted, y = .stdresid, data = m_back, position = "jitter")
```
...
I thinks if the same student takes two or more classes with the same instructor this will affect the outcome since independence will not be satisfied.

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.
    
Male instructors with english speaking, teach one credit class, not a minority and has a black and white picture will have high evaluation score.

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?
    
I will be comfortable generalizing my conclusions on similar kind/ size of universities and not all.
